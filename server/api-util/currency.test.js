...
describe('currency utils', () => {
  describe('convertDecimalJSToNumber(value, subUnitDivisor)', () => {
    ...
    it('Too big Decimals', () => {
      ...
      expect(() => convertDecimalJSToNumber(new Decimal(MIN_SAFE_INTEGER - 1), subUnitDivisor)).toThrowError(
        'Cannot represent Decimal.js value -9007199254740992 safely as a number'
      )
      expect(() => convertDecimalJSToNumber(new Decimal(MAX_SAFE_INTEGER + 1), subUnitDivisor)).toThrowError(
        'Cannot represent Decimal.js value 9007199254740992 safely as a number'
      )
    })

    it('wrong type', () => {
      expect(() => convertDecimalJSToNumber(0, subUnitDivisor)).toThrowError('Value must be a Decimal')
      expect(() => convertDecimalJSToNumber(10, subUnitDivisor)).toThrowError('Value must be a Decimal')
      expect(() => convertDecimalJSToNumber({}, subUnitDivisor)).toThrowError('Value must be a Decimal')
      expect(() => convertDecimalJSToNumber([], subUnitDivisor)).toThrowError('Value must be a Decimal')
      expect(() => convertDecimalJSToNumber(null, subUnitDivisor)).toThrowError('Value must be a Decimal')
    })
  })
  ...
  describe('convertMoneyToNumber(value)', () => {
    ...
    it('Wrong type of a parameter', () => {
      ...
      expect(() => getAmountAsDecimalJS(new Money('asdf', 'USD'))).toThrowError('[DecimalError] Invalid argument')
    })
  })
})

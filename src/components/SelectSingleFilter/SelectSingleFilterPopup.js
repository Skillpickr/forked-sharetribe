...
class SelectSingleFilterPopup extends Component {
  ...
  render() {
    const { rootClassName, className, label, options, queryParamNames, initialValues, contentPlacementOffset } =
      this.props

    const queryParamName = getQueryParamName(queryParamNames)
    const initialValue = initialValues && initialValues[queryParamNames] ? initialValues[queryParamNames] : null
          ...
          {options.map((option) => {
            ...
            return (
              <MenuItem key={option.key}>
                <button className={css.menuItem} onClick={() => this.selectOption(queryParamName, option.key)}>

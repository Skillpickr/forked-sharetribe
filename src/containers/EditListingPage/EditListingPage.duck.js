...
export default function reducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case REMOVE_LISTING_IMAGE: {
      ...
      // listing already
      const removedImageIds = state.images[id] ? state.removedImageIds : state.removedImageIds.concat(id)
    ...
    case DELETE_EXCEPTION_SUCCESS: {
      ...
      const availabilityExceptions = state.availabilityExceptions.filter((e) => e.id.uuid !== deletedExceptionId.uuid)
      return {
        ...state,
        availabilityExceptions,
        deleteExceptionInProgress: false

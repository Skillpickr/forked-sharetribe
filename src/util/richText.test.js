...
import { zwspAroundSpecialCharsSplit, linkifyOrWrapLinkSplit, wrapLongWord, richText } from './richText'
...
describe('richText', () => {
  ...
  describe('zwspAroundSpecialChars(word, optionalBreakingChars)', () => {
    ...
    it('should add zwsp to a word with slash', () => {
      expect(zwspAroundSpecialCharsSplit('word/another')).toEqual(['word', slashWithZWSP, 'another'])
    })
    ...
    it('should add zwsp to strings with given char (","), single char', () => {
      expect(zwspAroundSpecialCharsSplit('word,another', ',')).toEqual(['word', commaWithZWSP, 'another'])
    })
    ...
    it('should add zwsp to strings with given chars (",/"): multiple occurrences', () => {
      expect(zwspAroundSpecialCharsSplit('word/another/third,fourth and fifth,sixth', ',/')).toEqual([
        'word',
        slashWithZWSP,
        'another',
        slashWithZWSP,
    ...
    it('should add zwsp to strings with given chars ("?\\$") escapable chars not found', () => {
      expect(zwspAroundSpecialCharsSplit('word/another,third', '?\\$')).toEqual(['word/another,third'])
    })
  ...
  describe('wrapLongWord(word, key, longWordMinLength, longWordClass)', () => {
    it('should not add anything to short word', () => {
      const wrapper = shallow(
        <span>{wrapLongWord('word', 'key', { longWordMinLength: 10, longWordClass: 'longWord' })}</span>
      )
  ...
  describe('linkifyOrWrapLinkSplit(word, key, linkClass)', () => {
    it('should not add anything to words without links', () => {
      const wrapper = shallow(
        <span>{linkifyOrWrapLinkSplit('word', 'key', { linkify: true, linkClass: 'linkClass' })}</span>
      )
      expect(wrapper.html()).toEqual('<span>word</span>')
    })
  ...
  describe('richText(text, { longWordMinLength, longWordClass })', () => {
    ...
    it('should add span around a string with a single long word', () => {
      const wrapper = shallow(
        <span>{richText('word Pneumonoultramicroscopicsilicovolcanoconiosis is the longest word', options)}</span>
      )
      expect(wrapper.html()).toEqual(
    ...
    it('should add span around a string with multiple long words and containing slashes', () => {
      const wrapper = shallow(<span>{richText('Chars one/two/three - count until exhaustion…', options)}</span>)
      // <span>
    ...
    it('should add span around a string with a long word, containing slashes and a link', () => {
      const wrapper = shallow(
        <span>
          {richText('Chars one/two/three - count until exhaustion… and a random link: http://www.example.com', {
            ...options,
            linkify: true,
            linkClass: 'link'
          })}
    ...
    it('should add link inside non-whitespace-sequence (http://example.com)', () => {
      const wrapper = shallow(<span>{richText('Link: (http://example.com)', { ...options, linkify: true })}</span>)
      // <span>
    ...
    it('should not add span around a string if no linkify option is given', () => {
      const wrapper = shallow(
        <span>
          {richText('Chars one/two/three - count until exhaustion… and a random link: http://www.example.com', options)}
        </span>
      )
      // <span>

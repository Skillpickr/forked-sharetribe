...
const formatSelectedLabel = (minDurationOptions, minDuration, startDate, endDate) => {
  ...
  const minDurationOption =
    typeof minDuration === 'number'
      ...
      : null
  return minDurationOption ? `${startDate} - ${endDate}, ${minDurationOption.shortLabel}` : `${startDate} - ${endDate}`
}
...
const parseInitialValues = (initialValues) => {
  ...
  const initialDates = initialValues && startDate && endDate ? { dates: { startDate, endDate } } : { dates: null }

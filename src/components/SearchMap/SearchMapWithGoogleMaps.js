...
class SearchMapPriceLabelWithOverlay extends Component {
  ...
  render() {
    const { position, map, mapPaneName, isActive, className, listing, onListingClicked, mapComponentRefreshToken } =
      this.props
...
class SearchMapGroupLabelWithOverlay extends Component {
  ...
  render() {
    const { position, map, mapPaneName, isActive, className, listings, onListingClicked, mapComponentRefreshToken } =
      this.props
    return (
      <CustomOverlayView
        position={position}
...
const PriceLabelsAndGroups = (props) => {
  const { map, listings, activeListingId, infoCardOpen, onListingClicked, mapComponentRefreshToken } = props
...
const InfoCardComponent = (props) => {
  const { map, infoCardOpen, onListingInfoCardClicked, createURLToListing, mapComponentRefreshToken } = props
...
class SearchMapWithGoogleMaps extends Component {
  ...
  onIdle(e) {
    if (this.map) {
      ...
      const isHiddenByReusableMap =
        ...
        this.state.mapContainer.parentElement.classList.contains(this.props.reusableMapHiddenHandle)
      if (!isHiddenByReusableMap) {
        ...
        const viewportBoundsChanged =
          this.viewportBounds && viewportBounds && !hasSameSDKBounds(this.viewportBounds, viewportBounds)
  ...
  render() {
    ...
    return (
      <div id={id} ref={this.onMount} className={classNames(className, css.fullArea)} onClick={this.props.onClick}>
        {this.map ? (

...
module.exports = (err, user, req, res, idpClientId, idpId) => {
  ...
  if (!user) {
    log.error(new Error('Failed to fetch user details from identity provider'), 'fetching-user-data-from-idp-failed')
  ...
  return sdk
    ...
    .catch(() => {
      console.log('Authenticating with idp failed. User needs to confirm creating sign up in frontend.')

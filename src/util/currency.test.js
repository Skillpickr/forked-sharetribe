...
describe('currency utils', () => {
  ...
  describe('convertToDecimal(valueStr)', () => {
    ...
    it('Throws exceptions if string formatted parameter can not be converted to Decimal', () => {
      ...
      expect(() => convertToDecimal('123asdf')).toThrowError('[DecimalError] Invalid argument: 123asdf')
    })
  ...
  describe('convertDecimalToString(<Decimal>)', () => {
    it('converts Decimals to string', () => {
      ...
      expect(convertDecimalToString(new Decimal('999999999.99999999'))).toEqual('999999999.99999999')
      expect(convertDecimalToString(new Decimal('-999999999.99999999'))).toEqual('-999999999.99999999')
    })
    ...
    it('Throws exceptions if string formatted parameter is not a number', () => {
      expect(() => convertDecimalToString('asdf')).toThrowError('[DecimalError] Invalid argument: asdf')
      expect(() => convertDecimalToString('123asdf')).toThrowError('[DecimalError] Invalid argument: 123asdf')
    })
  })
  ...
  describe('convertUnitToSubUnit(value, subUnitDivisor)', () => {
    ...
    it('wrong type', () => {
      expect(() => convertUnitToSubUnit({}, subUnitDivisor)).toThrowError('Value must be either number or string')
      expect(() => convertUnitToSubUnit([], subUnitDivisor)).toThrowError('Value must be either number or string')
      expect(() => convertUnitToSubUnit(null, subUnitDivisor)).toThrowError('Value must be either number or string')
    })
  ...
  describe('convertMoneyToNumber(value)', () => {
    ...
    it('Wrong type of a parameter', () => {
      ...
      expect(() => convertMoneyToNumber(new Money('asdf', 'USD'))).toThrowError('[DecimalError] Invalid argument')
    })
  })

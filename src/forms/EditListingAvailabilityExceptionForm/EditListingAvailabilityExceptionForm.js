...
import { FieldDateInput, FieldRadioButton, FieldSelect, Form, IconArrowHead, PrimaryButton } from '../../components'
...
const extractDateFromFieldDateInput = (dateValue) => (dateValue && dateValue.date ? dateValue.date : null)
...
const startTimeOrStartOfSelectedEndDay = (selectedStartTimeAsDate, startOfSelectedEndDay) =>
  dateIsAfter(selectedStartTimeAsDate, startOfSelectedEndDay) ? selectedStartTimeAsDate : startOfSelectedEndDay
...
const getAvailableEndTimes = (selectedStartTime, selectedEndDate, selectedTimeRange, intl, timeZone) => {
  ...
  const limitStart = startTimeOrStartOfSelectedEndDay(selectedStartTimeAsDate, startOfSelectedEndDate)
...
const getAllTimeValues = (intl, timeZone, timeRanges, selectedStartDate, selectedStartTime, selectedEndDate) => {
  const startTimes = selectedStartTime
    ? []
    : getAvailableStartTimes(selectedStartDate, getTimeRanges(timeRanges, selectedStartDate, timeZone), intl, timeZone)
  ...
  const endTime = endTimes.length > 0 && endTimes[0] && endTimes[0].timestamp ? endTimes[0].timestamp : null
...
const isDayBlocked = (availableTimeRanges, timeZone) =>
  availableTimeRanges
    ? (day) =>
        !availableTimeRanges.find((timeRange) => isDayMomentInsideRange(day, timeRange.start, timeRange.end, timeZone))
    : () => false
...
const onExceptionStartDateChange = (value, timeRanges, props) => {
  ...
  const { startTime, endDate, endTime } = getAllTimeValues(intl, timeZone, timeRangesOnSelectedDate, startDate)
...
const onExceptionStartTimeChange = (value, timeRangesOnSelectedDate, props) => {
  ...
  const { endDate, endTime } = getAllTimeValues(intl, timeZone, timeRangesOnSelectedDate, startDate, value)
...
const onExceptionEndDateChange = (value, timeRangesOnSelectedDate, props) => {
  ...
  const { endTime } = getAllTimeValues(intl, timeZone, timeRangesOnSelectedDate, startDate, exceptionStartTime, endDate)
...
const Prev = (props) => {
  ...
  return dateIsAfter(prevMonthDate, currentMonthDate) ? <IconArrowHead direction="left" size="small" /> : null
}
  ...
  return (
    <FinalForm
      ...
      render={(formRenderProps) => {
        ...
        const availableTimeRanges = getAvailableTimeRangesForExceptions(availabilityExceptions, timeZone)

        const timeRangesOnSelectedDate = getTimeRanges(availableTimeRanges, exceptionStartDay, timeZone)

        const availableStartTimes = getAvailableStartTimes(exceptionStartDay, timeRangesOnSelectedDate, intl, timeZone)
        ...
        const hasData = availability && exceptionStartDate && exceptionStartTime && exceptionEndDate && exceptionEndTime
              ...
              <div className={css.formRow}>
                <div className={css.field}>
                  <FieldDateInput
                    ...
                    onChange={(value) => onExceptionStartDateChange(value, availableTimeRanges, formRenderProps)}
              ...
              <div className={css.formRow}>
                <div className={css.field}>
                  <FieldDateInput
                    ...
                    parse={parseFieldDateInput(timeZone)}
                    isDayBlocked={isDayBlocked(availableTimeRanges, timeZone)}
                    onChange={(value) => onExceptionEndDateChange(value, timeRangesOnSelectedDate, formRenderProps)}
                    onPrevMonthClick={() => handleMonthClick(prevMonthFn)}
                    onNextMonthClick={() => handleMonthClick(nextMonthFn)}
                    navNext={<Next currentMonth={currentMonth} timeZone={timeZone} />}
                    navPrev={<Prev currentMonth={currentMonth} timeZone={timeZone} />}

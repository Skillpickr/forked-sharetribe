...
class TokenInputFieldComponent extends Component {
  ...
  render() {
    const { rootClassName, className, country, formName, meta: formMeta, intl, disabled } = this.props

    if (!supportedCountries.includes(country)) {
      return (
        <div className={css.unsupportedCountryError}>
          <FormattedMessage id="StripeBankAccountTokenInputField.unsupportedCountry" values={{ country }} />
        </div>
      )
    }

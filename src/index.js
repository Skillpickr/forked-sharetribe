...
const render = (store, shouldHydrate) => {
  ...
  info
    .then(() => {
      ...
      return Promise.all([loadableReady(), store.dispatch(fetchAppAssets(config.appCdnAssets, cdnAssetsVersion))])
    })
    .then(([_, fetchedAssets]) => {
      const translations = fetchedAssets?.translations?.data || {}
      if (shouldHydrate) {
        ReactDOM.hydrate(<ClientApp store={store} hostedTranslations={translations} />, document.getElementById('root'))
      } else {
        ReactDOM.render(<ClientApp store={store} hostedTranslations={translations} />, document.getElementById('root'))
      }
    })
    .catch((e) => {

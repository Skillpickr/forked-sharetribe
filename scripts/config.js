...
const mandatoryVariables = (settings) => {
  ...
  const clientSecretDefaultMaybe =
    settings && settings.SHARETRIBE_SDK_CLIENT_SECRET !== '' ? { default: settings.SHARETRIBE_SDK_CLIENT_SECRET } : {}
  ...
  const mapBoxDefaultMaybe =
    settings && settings.REACT_APP_MAPBOX_ACCESS_TOKEN !== '' ? { default: settings.REACT_APP_MAPBOX_ACCESS_TOKEN } : {}
...
${chalk.dim('Client ID is needed for connecting with Flex API. You can find your client ID from Flex Console.')}
`,
...
const advancedSettings = (settings) => {
  ...
  const availabilityDefault = settings ? settings.REACT_APP_AVAILABILITY_ENABLED.toLowerCase() === 'true' : true
  const searchesDefault = settings ? settings.REACT_APP_DEFAULT_SEARCHES_ENABLED.toLowerCase() === 'true' : true
...
${chalk.dim('This setting enables the Availability Calendar for listings. The default value for this setting is true.')}
`,

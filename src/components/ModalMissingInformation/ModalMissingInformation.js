...
class ModalMissingInformation extends Component {
  ...
  handleMissingInformationReminder(currentUser, currentUserHasListings, currentUserHasOrders, newLocation) {
    ...
    const whitelistedPaths = MISSING_INFORMATION_MODAL_WHITELIST.map((page) => pathByRouteName(page, routes))
  ...
  render() {
    ...
    const closeButtonMessage = <FormattedMessage id="ModalMissingInformation.closeVerifyEmailReminder" />
